  ...
  try {
    ...
    if (action === "flag-stalled") {
      ...
      if (elapsedMs < 10 * 60 * 1000) {
        res.status(200).json({ ok: true, skipped: "too-early" });
        return;
      }
      ...
      if (!claimRes.ok || !Array.isArray(claimed) || !claimed.length) {
        res.status(200).json({ ok: true, skipped: "race-lost" });
        return;
      }
    ...
    if (action === "request-submission") {
      ...
      res.status(200).json({ uploadUrl, anonKey: SUPABASE_ANON_KEY, ticket, bucket, path });
      return;
    }
    ...
    // ── DOWNLOAD (streams file through proxy — Supabase URL never exposed) ──
    if (action === "download") {
